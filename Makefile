...
CLEAN=rm -rf
# Simulator
SIMAVRDIR=/usr/local/Cellar/simavr/HEAD-ab4c17c/
SIMAVR=simavr
...
# based on the link above added '.brk' after 'sed -i'
test: $(PATHO)main.elf
	@sed -i .brk "s/break main.c:.*/break main.c:$(WHILELINENO)/" $(INITDEBUGGER)
	$(SIMAVR) -g -mmcu=$(MMCU) -f $(FREQ) $< &
	$(GDB) $(TESTING)
	@pkill simavr
